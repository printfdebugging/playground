...
{
    _tmp_entities.push_back(entity);
}
...
void World::clear()
{
    ...
    for (Entity* entity : _tmp_entities)
    {
        delete entity;
    }
...
{
    return _entities.size() + _tmp_entities.size();
}
...
{
    if (_tmp_entities.size() > 0)
        _entities.merge(_tmp_entities);
