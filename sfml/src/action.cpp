...
{
    /** such memcpy operations are only valid when the
     *  type being copied doesn't have any pointers. */
...
Action::Action(const sf::Mouse::Button& mouse, int type)
    : _type(type)
{
    _event.type               = sf::Event::EventType::MouseButtonPressed;
    _event.mouseButton.button = mouse;
}
